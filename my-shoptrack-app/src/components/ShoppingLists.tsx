...
import { format, isWithinInterval, addDays, parseISO, differenceInDays } from 'date-fns';
...
const ShoppingLists: React.FC<ShoppingListsProps> = ({ searchTerm }) => {
  ...
  const getProgress = (deadline: string | undefined) => {
    if (!deadline) return { width: 100, color: 'bg-green-500' };
  
    const deadlineDate = parseISO(deadline);
    const today = new Date();
    const totalDuration = differenceInDays(deadlineDate, today);
    ...
    const progress = totalDuration > 0 ? ((7 - totalDuration) / 7) * 100 : 100;
  
    return { width: progress > 100 ? 100 : progress, color };
  };
  ...
  return (
    ...
    {filteredLists.length === 0 ? (
      <Alert icon={<Icon />}>Nie znaleziono żadnych list zakupów.</Alert>
    ) : (
      filteredLists
        .sort((a, b) => Number(b.pinned) - Number(a.pinned))
        .map(list => {
          const progressBar = getProgress(list.deadline);
              ...
              <div role="button" className="flex items-center justify-between w-full p-3 leading-tight transition-all rounded-lg outline-none text-start hover:bg-blue-gray-50 hover:bg-opacity-80 hover:text-blue-gray-900 focus:bg-blue-gray-50 focus:bg-opacity-80 focus:text-blue-gray-900 active:bg-blue-gray-50 active:bg-opacity-80 active:text-blue-gray-900">
                <Link to={`/list/${list.id}`} className="flex items-center">
                  <div className="grid mr-4 place-items-center">
                    <svg className="h-12 w-12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"> <circle cx="12" cy="12" r="10"></circle> <line x1="12" y1="16" x2="12" y2="12"></line> <line x1="12" y1="8" x2="12.01" y2="8"></line> </svg>
                  </div>
                  <div>
                    ...
                    <p className="block font-sans text-sm antialiased font-normal leading-normal text-gray-700">
                      {list.deadline && isUpcoming(list.deadline) ? `Termin: ${format(parseISO(list.deadline), 'PPP', { locale: pl })}` : ''}
                    </p>

                    <div className="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700 mt-4">
                      <div className={`${progressBar.color} h-2.5 rounded-full`} style={{ width: `${progressBar.width}%` }}></div>
                    </div>
                  </div>
                </Link>
                <div className="flex items-center justify-start space-x-2">
                {list.pinned ? (
                    <IconButton variant="text" color="blue-gray" onClick={() => handleUnpinList(list.id)} placeholder={undefined} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}>
                      <MdPushPin className="h-6 w-6 blue-gray-500" />
                    </IconButton>
                  ) : (
                    <IconButton variant="text" color="blue-gray" onClick={() => handlePinList(list.id)} placeholder={undefined} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}>
                      <MdPushPin className="h-6 w-6 blue-gray-500" />
                    </IconButton>
                  )}
                  ...
                  <svg className="h-6 w-6 blue-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path> </svg>
                  </IconButton>
                </div>
              </div>
            </nav>
